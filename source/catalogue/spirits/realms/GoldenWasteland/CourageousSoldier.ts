...
export default new StandardSpirit({
	...
	offer: {
		current: new Collection<number, FriendshipTreeItem>()
			...
			.set(1 << 1, { name: "Hair", cost: { hearts: 4 }, emoji: HAIR_EMOJIS.Hair23 })
			...
			.set(1 << 4, { name: "Wing buff 1", cost: { ascendedCandles: 2 }, emoji: MISCELLANEOUS_EMOJIS.WingBuff })
			.set(1 << 5, { name: "Blessing 2", cost: { candles: 5 }, emoji: MISCELLANEOUS_EMOJIS.Blessing2 })
			.set(1 << 6, { name: "Cape 1", cost: { hearts: 15 }, emoji: CAPE_EMOJIS.Cape09 })
			.set(1 << 7, { name: "Wing buff 2", cost: { ascendedCandles: 6 }, emoji: MISCELLANEOUS_EMOJIS.WingBuff })
			.set(1 << 8, { name: "Cape 2", cost: { hearts: 45 }, emoji: CAPE_EMOJIS.Cape47 }),
	},
});
