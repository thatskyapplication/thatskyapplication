...
export default new StandardSpirit({
	...
	offer: {
		current: new Collection<number, FriendshipTreeItem>()
			...
			.set(1 << 1, { name: `${emote} 2`, cost: { candles: 2 }, emoji: emoteEmoji })
			.set(1 << 2, { name: "Blessing 1", cost: { candles: 1 }, emoji: MISCELLANEOUS_EMOJIS.Blessing1 })
			.set(1 << 3, { name: "Outfit", cost: { hearts: 2 }, emoji: OUTFIT_EMOJIS.Outfit05 })
			...
			.set(1 << 6, { name: `${emote} 3`, cost: { candles: 3 }, emoji: emoteEmoji })
			.set(1 << 7, { name: `${emote} 4`, cost: { candles: 3 }, emoji: emoteEmoji })
			.set(1 << 8, { name: "Blessing 2", cost: { candles: 5 }, emoji: MISCELLANEOUS_EMOJIS.Blessing2 })
			.set(1 << 9, { name: "Hair", cost: { hearts: 5 }, emoji: HAIR_EMOJIS.Hair10 }),
	},
});
