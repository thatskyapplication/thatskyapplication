...
export default new StandardSpirit({
	...
	offer: {
		current: new Collection<number, FriendshipTreeItem>()
			...
			.set(1 << 4, { name: "Wing buff", cost: { ascendedCandles: 1 }, emoji: MISCELLANEOUS_EMOJIS.WingBuff })
			.set(1 << 5, { name: `${emote} 3`, cost: { candles: 5 }, emoji: emoteEmoji })
			.set(1 << 6, { name: `${emote} 4`, cost: { candles: 5 }, emoji: emoteEmoji })
			.set(1 << 7, { name: "Blessing 2", cost: { candles: 5 }, emoji: MISCELLANEOUS_EMOJIS.Blessing2 })
			.set(1 << 8, { name: "Face accessory", cost: { hearts: 3 }, emoji: faceAccessoryEmoji }),
	},
});
