...
export default new StandardSpirit({
	...
	offer: {
		current: [
			...
			{ name: "Cape 1", bit: 1 << 3, cost: { hearts: 3 }, emoji: CAPE_EMOJIS.Cape04 },
			{ name: "Heart", bit: 1 << 4, cost: { candles: 3 }, emoji: MISCELLANEOUS_EMOJIS.Heart },
			{ name: "Wing buff 1", bit: 1 << 5, cost: { ascendedCandles: 1 }, emoji: MISCELLANEOUS_EMOJIS.WingBuff },
			{ name: `${emote} 3`, bit: 1 << 6, cost: { candles: 2 }, emoji: emoteEmoji },
			{ name: `${emote} 4`, bit: 1 << 7, cost: { candles: 2 }, emoji: emoteEmoji },
			{ name: "Outfit", bit: 1 << 8, cost: { hearts: 4 }, emoji: OUTFIT_EMOJIS.Outfit04 },
			{ name: "Blessing 2", bit: 1 << 9, cost: { candles: 5 }, emoji: MISCELLANEOUS_EMOJIS.Blessing2 },
			{ name: "Wing buff 2", bit: 1 << 10, cost: { ascendedCandles: 3 }, emoji: MISCELLANEOUS_EMOJIS.WingBuff },
			{ name: "Cape 2", bit: 1 << 11, cost: { hearts: 9 }, emoji: CAPE_EMOJIS.Cape38 },
		],
	},
});
