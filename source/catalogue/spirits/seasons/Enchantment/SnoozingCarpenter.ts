...
export default new SeasonalSpirit({
	...
	offer: {
		...
		seasonal: [
			...
			{ name: "Cape", bit: 1 << 9, cost: { seasonalCandles: 14 }, emoji: capeEmoji },
			{ name: "Blessing 2", bit: 1 << 8, emoji: blessing2 },
			{ name: "Seasonal heart", bit: 1 << 4, cost: { seasonalCandles: 3 }, emoji: SEASON_EMOJIS.EnchantmentHeart },
		],
		current: [
			{ name: `${emote} 1`, bit: 1 << 0, emoji: emoteEmoji },
			{ name: `${emote} 2`, bit: 1 << 1, cost: { hearts: 4 }, emoji: emoteEmoji },
			{ name: "Blessing 1", bit: 1 << 2, cost: { candles: 5 }, emoji: blessing2 },
			{ name: "Hair", bit: 1 << 3, cost: { candles: 34 }, emoji: hairEmoji },
