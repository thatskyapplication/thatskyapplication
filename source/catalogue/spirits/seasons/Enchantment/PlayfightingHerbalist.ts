...
export default new SeasonalSpirit({
	...
	offer: {
		seasonal: new Collection<number, ItemRaw>()
			...
			.set(1 << 8, { name: "Blessing 3", cost: { seasonalCandles: 16 }, emoji: blessing2 })
			.set(1 << 6, { name: `${action} 2`, emoji: actionEmoji })
			.set(1 << 7, { name: "Music sheet", cost: { seasonalCandles: 18 }, emoji: musicSheet })
			.set(1 << 9, { name: "Hair", emoji: hairEmoji })
			.set(1 << 11, { name: "Cape", cost: { seasonalCandles: 20 }, emoji: capeEmoji })
			.set(1 << 12, { name: "Blessing 4", emoji: blessing2 })
		...
		current: new Collection<number, ItemRaw>()
			...
			.set(1 << 6, { name: `${action} 2`, cost: { hearts: 10 }, emoji: actionEmoji })
			.set(1 << 7, { name: "Music sheet", cost: { candles: 15 }, emoji: musicSheet })
			.set(1 << 8, { name: "Blessing 3", cost: { candles: 5 }, emoji: blessing2 })
			.set(1 << 9, { name: "Hair", cost: { candles: 42 }, emoji: hairEmoji })
			.set(1 << 10, { name: "Orb", cost: { candles: 20 }, emoji: placeablePropEmoji })
