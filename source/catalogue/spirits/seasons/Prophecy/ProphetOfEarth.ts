...
export default new SeasonalSpirit({
	...
	offer: {
		...
		seasonal: new Collection<number, FriendshipTreeItem>()
			...
			.set(1 << 9, { name: "Music sheet", cost: { seasonalCandles: 21 }, emoji: musicSheet })
			.set(1 << 10, { name: "Blessing 2", cost: null, emoji: blessing2 })
			.set(1 << 11, { name: "Cape", cost: { seasonalCandles: 27 }, emoji: capeEmoji })
			.set(1 << 12, { name: "Mask", cost: null, emoji: maskEmoji })
		...
		current: new Collection<number, FriendshipTreeItem>()
			...
			.set(1 << 9, { name: "Music sheet", cost: { candles: 15 }, emoji: musicSheet })
			.set(1 << 10, { name: "Blessing 2", cost: { candles: 5 }, emoji: blessing2 })
			.set(1 << 11, { name: "Cape", cost: { candles: 75 }, emoji: capeEmoji })
			.set(1 << 12, { name: "Mask", cost: { candles: 44 }, emoji: maskEmoji }),
	},
	visits: {
