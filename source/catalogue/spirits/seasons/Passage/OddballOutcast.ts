...
export default new SeasonalSpirit({
	...
	offer: {
		...
		seasonal: new Collection<number, ItemRaw>()
			...
			.set(1 << 2, { name: "Hair", cost: { seasonalCandles: 14 }, emoji: hairEmoji })
			.set(1 << 3, { name: "Blessing 1", cost: null, emoji: blessing3 })
			.set(1 << 4, { name: "Blessing 2", cost: { seasonalCandles: 18 }, emoji: blessing3 })
			.set(1 << 5, { name: "Neck accessory", cost: null, emoji: necklaceEmoji })
			.set(1 << 6, { name: `${emote} 3`, cost: { seasonalCandles: 24 }, emoji: emoteEmoji })
