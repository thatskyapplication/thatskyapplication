...
export default new SeasonalSpirit({
	...
	offer: {
		current: new Collection<number, FriendshipTreeItem>()
			...
			.set(1 << 11, { name: "Wing buff", cost: { ascendedCandles: 2 }, emoji: MISCELLANEOUS_EMOJIS.AscendedCandle })
			.set(1 << 4, { name: `${emote} 3`, cost: { hearts: 3 }, emoji: emoteEmoji })
			.set(1 << 5, { name: `${emote} 4`, cost: { hearts: 6 }, emoji: emoteEmoji })
			.set(1 << 7, { name: "Mask", cost: { candles: 48 }, emoji: maskEmoji })
			.set(1 << 6, { name: "Blessing 2", cost: { candles: 5 }, emoji: blessing2 })
			.set(1 << 9, { name: "Lute", cost: { candles: 70 }, emoji: heldProp })
			.set(1 << 8, { name: "Cape", cost: { candles: 75 }, emoji: capeEmoji }),
		seasonal: new Collection<number, FriendshipTreeItem>()
			...
			.set(1 << 6, { name: "Blessing 2", cost: { seasonalCandles: 21 }, emoji: blessing2 })
			.set(1 << 7, { name: "Mask", cost: null, emoji: maskEmoji })
			.set(1 << 8, { name: "Cape", cost: { seasonalCandles: 27 }, emoji: capeEmoji })
			.set(1 << 9, { name: "Lute", cost: null, emoji: heldProp })
			.set(1 << 10, { name: "Seasonal heart", cost: { seasonalCandles: 3 }, emoji: SEASON_EMOJIS.DreamsHeart }),
	},
	visits: {
