...
export default new SeasonalSpirit({
	...
	offer: {
		...
		seasonal: new Collection<number, ItemRaw>()
			...
			.set(1 << 2, { name: "Blessing", cost: { seasonalCandles: 14 }, emoji: blessing2 })
			.set(1 << 10, { name: "Guitar", emoji: heldProp })
			.set(1 << 6, { name: `${emote} 3`, cost: { seasonalCandles: 16 }, emoji: emoteEmoji })
			.set(1 << 7, { name: `${emote} 4`, emoji: emoteEmoji })
			.set(1 << 3, { name: "Mask", cost: { seasonalCandles: 18 }, emoji: maskEmoji })
			.set(1 << 9, { name: "Cape", emoji: capeEmoji })
		...
		current: new Collection<number, ItemRaw>()
			...
			.set(1 << 10, { name: "Guitar", cost: { candles: 75 }, emoji: heldProp }),
	},
	visits: {
