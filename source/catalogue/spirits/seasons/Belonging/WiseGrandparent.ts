import { Collection } from "discord.js";
import type { DateTime } from "luxon";
import { type SeasonalSpiritVisitCollectionKey, SeasonalSpirit } from "../../../../Structures/Spirits.js";
import { RealmName } from "../../../../Utility/Constants.js";
import type { ItemRaw } from "../../../../Utility/catalogue.js";
import { SeasonName } from "../../../../Utility/catalogue.js";
import { skyDate } from "../../../../Utility/dates.js";
import {
	CAPE_EMOJIS,
	MASK_EMOJIS,
	MISCELLANEOUS_EMOJIS,
	SEASON_EMOJIS,
	SMALL_PLACEABLE_PROPS_EMOJIS,
} from "../../../../Utility/emojis.js";
import { SpiritName, SpiritStance, SpiritStanceToEmoji } from "../../../../Utility/spirits.js";

const stance = SpiritStance.Wise;
const stanceEmoji = SpiritStanceToEmoji[stance];
const blessing1 = MISCELLANEOUS_EMOJIS.Blessing1;
const blessing2 = MISCELLANEOUS_EMOJIS.Blessing2;
const musicSheet = MISCELLANEOUS_EMOJIS.MusicSheet;
const maskEmoji = MASK_EMOJIS.Mask19;
const capeEmoji = CAPE_EMOJIS.Cape22;
const placeablePropEmoji = SMALL_PLACEABLE_PROPS_EMOJIS.SmallPlaceableProp09;

export default new SeasonalSpirit({
	name: SpiritName.WiseGrandparent,
	season: SeasonName.Belonging,
	stance,
	realm: RealmName.VaultOfKnowledge,
	offer: {
		seasonal: new Collection<number, ItemRaw>()
			.set(1 << 0, { name: `${stance} stance`, emoji: stanceEmoji })
			.set(1 << 3, { name: "Blessing 1", cost: { seasonalCandles: 10 }, emoji: blessing1 })
			.set(1 << 1, { name: "Music sheet", emoji: musicSheet })
			.set(1 << 5, { name: "Blessing 2", cost: { seasonalCandles: 12 }, emoji: blessing2 })
			.set(1 << 9, { name: "Blessing 3", emoji: blessing2 })
			.set(1 << 8, { name: "Mask", cost: { seasonalCandles: 14 }, emoji: maskEmoji })
			.set(1 << 10, { name: "Blessing 4", emoji: blessing2 })
			.set(1 << 11, { name: "Blessing 5", cost: { seasonalCandles: 16 }, emoji: blessing2 })
			.set(1 << 6, { name: "Cape", emoji: capeEmoji })
			.set(1 << 2, { name: "Seasonal heart", cost: { seasonalCandles: 3 }, emoji: SEASON_EMOJIS.BelongingHeart }),
		current: new Collection<number, ItemRaw>()
			.set(1 << 0, { name: `${stance} stance`, emoji: stanceEmoji })
			.set(1 << 1, { name: "Music sheet", cost: { candles: 15 }, emoji: musicSheet })
			.set(1 << 2, { name: "Heart", cost: { candles: 3 }, emoji: MISCELLANEOUS_EMOJIS.Heart })
			.set(1 << 3, { name: "Blessing 1", cost: { candles: 5 }, emoji: blessing2 })
			.set(1 << 4, { name: "Wing buff", cost: { ascendedCandles: 2 }, emoji: MISCELLANEOUS_EMOJIS.WingBuff })
			.set(1 << 5, { name: "Blessing 2", cost: { candles: 5 }, emoji: blessing2 })
			.set(1 << 6, { name: "Cape", cost: { candles: 70 }, emoji: capeEmoji })
			.set(1 << 7, { name: "Prop", cost: { candles: 10 }, emoji: placeablePropEmoji })
			.set(1 << 8, { name: "Mask", cost: { candles: 48 }, emoji: maskEmoji }),
	},
	visits: {
		travelling: new Collection<SeasonalSpiritVisitCollectionKey, DateTime>()
			.set(15, skyDate(2_020, 8, 6))
			.set(48, skyDate(2_021, 11, 11))
			.set(100, skyDate(2_023, 11, 9)),
	},
});
