...
export default new SeasonalSpirit({
	...
	offer: {
		...
		seasonal: new Collection<number, ItemsData>()
			...
			.set(1 << 4, { item: "Mask", cost: { seasonalCandles: 8 } })
			.set(1 << 5, { item: "Blessing 2", cost: null })
			.set(1 << 6, { item: `${emote} 3`, cost: { seasonalCandles: 16 }, emoji: emoteEmoji })
			.set(1 << 7, { item: `${emote} 4`, cost: null, emoji: emoteEmoji })
			.set(1 << 8, { item: "Cape", cost: { seasonalCandles: 20 } })
			.set(1 << 9, { item: "Blessing 3", cost: null })
			.set(1 << 10, { item: "Seasonal heart", cost: { seasonalCandles: 3 }, emoji: MISCELLANEOUS_EMOJIS.HeartAbyss }),
	},
});
