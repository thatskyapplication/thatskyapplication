	...
	return (
		<>
			<Popover.Root>
				...
				>
					<CalendarEntryIcons className="h-3.5 w-3.5" emojiIds={entry.iconEmojiIds} />
						...
						<Popover.Popup className="z-50 w-72 max-w-[calc(100vw-1rem)] rounded-xl border border-gray-200 bg-white p-4 shadow-xl dark:border-gray-700 dark:bg-gray-900">
							...
							{entry.times.length > 0 && (
								<div className="mt-2 flex flex-wrap gap-1">
									<CalendarTimeChips times={entry.times} />
								</div>
							)}
							{entry.spiritLinks && (
							...
							<CalendarEntryLinks
								className="mt-3 flex flex-wrap items-center gap-3 text-sm font-medium"
								entry={entry}
								iconClassName="h-3.5 w-3.5"
								linkClassName="regular-link"
							/>
						</Popover.Popup>
					</Popover.Positioner>
				</Popover.Portal>
