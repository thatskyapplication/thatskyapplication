...
const ISSUE_DESCRIPTION_FILE_UPLOAD_CUSTOM_ID = "ISSUE_DESCRIPTION_FILE_UPLOAD_CUSTOM_ID" as const;
		...
		components: [
			...
			},
			{
				...
				component: {
					...
					min_values: 0,
					max_values: 10,
					custom_id: ISSUE_DESCRIPTION_FILE_UPLOAD_CUSTOM_ID,
					required: false,
				},
...
export async function issueSubmission(interaction: APIModalSubmitInteraction) {
	...
	const attachments = components
		.getFileUploadValeus(ISSUE_DESCRIPTION_FILE_UPLOAD_CUSTOM_ID)
		.filter((attachment) =>
